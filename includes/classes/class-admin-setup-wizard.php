...
class qa_Admin_Setup_Wizard {
	...
	public function qa_notice_setup_wizard() {
		...
		if( get_option( 'qa_complete_setting_wizard', 'no' ) == 'no' ) {
			...
			<div id="message" class="updated">
				<p><?php _e( '<strong>Welcome to Question Answer</strong> &#8211; You did not run the setup wizard yet. You can do that now.', QA_TEXTDOMAIN ); ?></p>
				<p class="submit">
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=qa-setup' ) ); ?>" class="button-primary"><?php _e( 'Run setup wizard', QA_TEXTDOMAIN ); ?></a> 
					<a class="button-secondary skip" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'qa-hide-notice', 'wizard' ), 'qa_hide_notices_nonce', '_qa_notice_nonce' ) ); ?>"><?php _e( 'Skip setup', QA_TEXTDOMAIN ); ?></a>
				</p>
			</div>
			<?php
		}
	...
	public function setup_wizard_header() {
		...
		<head>
			...
			<title><?php _e( 'Question Answer &rsaquo; Setup sizard', QA_TEXTDOMAIN ); ?></title>
	...
	public function setup_wizard_footer() {
		?>
			<?php if ( 'next_steps' === $this->step ) : ?>
				<div class="qa-return-to-dashboard"><a href="<?php echo esc_url( admin_url() ); ?>"><?php _e( 'Return to the WordPress dashboard', QA_TEXTDOMAIN ); ?></a></div>
			<?php endif; ?>
			</body>
		</html>
		<?php
	...
	public function qa_setup_pages() {
		?>
		<h1><?php _e( 'Page setup', QA_TEXTDOMAIN ); ?></h1>
		<form method="post">
			<p><?php printf( __( 'Your website needs a few essential %spages%s. The following will be created automatically (if they do not already exist):', QA_TEXTDOMAIN ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
			<table class="qa-setup-pages" cellspacing="0">
				<thead>
					<tr>
						<th class="page-name"><?php _e( 'Page name', QA_TEXTDOMAIN ); ?></th>
						<th class="page-description"><?php _e( 'Description', QA_TEXTDOMAIN ); ?></th>
					</tr>
				</thead>
				<tbody>
					<tr>
						<td class="page-name"><?php echo _x( 'Question submission', 'Page title', QA_TEXTDOMAIN ); ?></td>
						<td><?php _e( 'This page will display a form where visitor can submit new question.', QA_TEXTDOMAIN ); ?></td>
					</tr>
					<tr>
						<td class="page-name"><?php echo _x( 'Question archive', 'Page title', QA_TEXTDOMAIN ); ?></td>
						<td><?php _e( 'This page will show all the live questions.', QA_TEXTDOMAIN ); ?></td>
					</tr>
					<tr>
						<td class="page-name"><?php echo _x( 'My account', 'Page title', QA_TEXTDOMAIN ); ?></td>
						<td><?php _e( 'This page will show your user account info.', QA_TEXTDOMAIN ); ?></td>
					</tr>
				</tbody>
			</table>

			<p><?php printf( __( 'Once created, these pages can be managed from your admin dashboard on the %sPages screen%s. <br> You can control which pages are shown on your website via %sAppearance > Menus%s.', QA_TEXTDOMAIN ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>', '<a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" target="_blank">', '</a>' ); ?></p>

			<p class="qa-setup-actions step">
				
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="qa-button-skip"><?php _e( 'Skip this step', QA_TEXTDOMAIN ); ?></a>
				<input type="submit" class="qa-button-next qa-input" value="<?php esc_attr_e( 'Continue', QA_TEXTDOMAIN ); ?>" name="save_step" />
	...
	public function qa_setup_question() {
		...
		?>
		<h1><?php _e( 'Question settings', QA_TEXTDOMAIN ); ?></h1>
		<form method="post">
			<table class="form-table">

				<tr>
					<th scope="row"><label for="qa_question_item_per_page"><?php _e( 'Item per page', QA_TEXTDOMAIN ); ?></label></th>
					<td>
				...
				<tr>
					<th scope="row"><label for="qa_question_excerpt_length"><?php _e( 'Excerpt length in question archive', QA_TEXTDOMAIN ); ?></label></th>
					<td>
				...
				<tr>
					<th scope="row"><label for="qa_account_required_post_question"><?php _e( 'Account required to submit question?', QA_TEXTDOMAIN ); ?></label></th>
					<td>
						<select id="qa_account_required_post_question" name="qa_account_required_post_question" class="qa-enhanced-select">
							<option value="yes" <?php selected( $qa_account_required_post_question, 'yes' ); ?>><?php echo __( 'Yes', QA_TEXTDOMAIN ); ?></option>
							<option value="no" <?php selected( $qa_account_required_post_question, 'no' ); ?>><?php echo __( 'No', QA_TEXTDOMAIN ); ?></option>
						</select>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="qa_submitted_question_status"><?php _e( 'New submitted question status ?', QA_TEXTDOMAIN ); ?></label></th>
					<td>
						<select id="qa_submitted_question_status" name="qa_submitted_question_status" class="qa-enhanced-select">
							<option value="pending" <?php selected( $qa_submitted_question_status, 'pending' ); ?>><?php echo __( 'Pending', QA_TEXTDOMAIN ); ?></option>
							<option value="draft" <?php selected( $qa_submitted_question_status, 'draft' ); ?>><?php echo __( 'Draft', QA_TEXTDOMAIN ); ?></option>
							<option value="publish" <?php selected( $qa_submitted_question_status, 'publish' ); ?>><?php echo __( 'Published', QA_TEXTDOMAIN ); ?></option>
							<option value="private" <?php selected( $qa_submitted_question_status, 'private' ); ?>><?php echo __( 'Privated', QA_TEXTDOMAIN ); ?></option>
							<option value="trash" <?php selected( $qa_submitted_question_status, 'trash' ); ?>><?php echo __( 'Trashed', QA_TEXTDOMAIN ); ?></option>
						</select>
					</td>
				</tr>
			</table>
	...
	public function qa_setup_answer() {
		...
		<form method="post">
			<table class="form-table">
				...
				<tr>
					<th scope="row"><label for="qa_answer_item_per_page"><?php _e( 'Item per page', QA_TEXTDOMAIN ); ?></label></th>
					<td>
				...
				<tr>
					<th scope="row"><label for="qa_account_required_post_answer"><?php _e( 'Account required to submit answer?', QA_TEXTDOMAIN ); ?></label></th>
					<td>
						<select id="qa_account_required_post_answer" name="qa_account_required_post_answer" class="qa-enhanced-select">
							<option value="yes" <?php selected( $qa_account_required_post_answer, 'yes' ); ?>><?php echo __( 'Yes', QA_TEXTDOMAIN ); ?></option>
							<option value="no" <?php selected( $qa_account_required_post_answer, 'no' ); ?>><?php echo __( 'No', QA_TEXTDOMAIN ); ?></option>
						</select>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="qa_submitted_answer_status"><?php _e( 'New submitted answer status ?', QA_TEXTDOMAIN ); ?></label></th>
					<td>
						<select id="qa_submitted_answer_status" name="qa_submitted_answer_status" class="qa-enhanced-select">
							<option value="pending" <?php selected( $qa_submitted_answer_status, 'pending' ); ?>><?php echo __( 'Pending', QA_TEXTDOMAIN ); ?></option>
							<option value="draft" <?php selected( $qa_submitted_answer_status, 'draft' ); ?>><?php echo __( 'Draft', QA_TEXTDOMAIN ); ?></option>
							<option value="publish" <?php selected( $qa_submitted_answer_status, 'publish' ); ?>><?php echo __( 'Published', QA_TEXTDOMAIN ); ?></option>
							<option value="private" <?php selected( $qa_submitted_answer_status, 'private' ); ?>><?php echo __( 'Privated', QA_TEXTDOMAIN ); ?></option>
							<option value="trash" <?php selected( $qa_submitted_answer_status, 'trash' ); ?>><?php echo __( 'Trashed', QA_TEXTDOMAIN ); ?></option>
						</select>
					</td>
				</tr>
			</table>
	...
	public function qa_setup_ready() {
		...
		<h1><?php _e( 'Your website is ready!', QA_TEXTDOMAIN ); ?></h1>
